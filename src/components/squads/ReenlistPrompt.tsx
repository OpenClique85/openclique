...
export function ReenlistPrompt({ questId, userId, onResponse }: ReenlistPromptProps) {
  ...
  useEffect(() => {
    const fetchSquadMembers = async () => {
      ...
      // Get all squad members with their profile names
      ...
      if (members && members.length > 0) {
        ...
        if (profiles) {
          ...
          setSquadMembers(membersWithNames);
        }
      }
      setIsLoading(false);
    };
  ...
  if (squadMembers.length === 0) {
    return null; // Don't show if no squad found
  }
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-display font-semibold text-foreground">
              Want to quest with this group again?
            </h3>
            <p className="text-sm text-muted-foreground">
              Keep the adventure going with your squad!
            </p>
          </div>
        </div>
