-- Fix confirm_warm_up_readiness to use instance_id instead of quest_id
...
BEGIN
  ...
  -- Log event with instance_id (not quest_id)
  INSERT INTO quest_event_log (instance_id, event_type, target_user_id, actor_type, payload)
  SELECT COALESCE(qs.instance_id, qs.quest_id), 'readiness_confirmed', auth.uid(), 'user', jsonb_build_object('squad_id', p_squad_id)
  FROM quest_squads qs WHERE qs.id = p_squad_id
  AND COALESCE(qs.instance_id, qs.quest_id) IS NOT NULL;
...
-- Fix submit_warm_up_prompt to use instance_id
...
BEGIN
  ...
  -- Log event with instance_id (not quest_id)
  INSERT INTO quest_event_log (instance_id, event_type, target_user_id, actor_type, payload)
  SELECT COALESCE(qs.instance_id, qs.quest_id), 'prompt_answered', auth.uid(), 'user', jsonb_build_object('squad_id', p_squad_id)
  FROM quest_squads qs WHERE qs.id = p_squad_id
  AND COALESCE(qs.instance_id, qs.quest_id) IS NOT NULL;
...
-- Fix check_squad_readiness to use instance_id
...
DECLARE
  ...
  v_instance_id UUID;
BEGIN
  -- Get squad status, instance_id, and min ready percentage
  SELECT qs.status, COALESCE(qs.instance_id, qs.quest_id), COALESCE(qi.warm_up_min_ready_pct, 100)
  INTO v_squad_status, v_instance_id, v_min_ready_pct
  FROM quest_squads qs
  LEFT JOIN quest_instances qi ON qi.id = COALESCE(qs.instance_id, qs.quest_id)
  ...
  IF v_is_ready AND v_squad_status = 'warming_up' THEN
    ...
    -- Log event with instance_id
    IF v_instance_id IS NOT NULL THEN
      INSERT INTO quest_event_log (instance_id, event_type, actor_type, payload)
      VALUES (v_instance_id, 'squad_ready_for_review', 'system', jsonb_build_object('squad_id', p_squad_id, 'ready_count', v_ready_members, 'total', v_total_members));
    END IF;
  END IF;
